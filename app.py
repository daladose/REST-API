...
def get_user_by_id(user_id):
    ...
    results = app.db.select(
        'SELECT users.id, users.name, users.created, faces.id, faces.user_id, faces.filename, faces.created FROM '
        'users LEFT JOIN faces ON faces.user_id = users.id WHERE users.id = ?',
        [user_id])
...
def train():
    ...
    if 'file' not in request.files:
        ...
        if file.mimetype not in app.config['file_allowed']:
            ...
            if user_id:
                print ("User saved in database", name, user_id, created)
                # user has been save with user_id and new we need save faces table as well
